...
class EC2CloudProvider(CloudProvider):
    ...
    def terminate_instances(self, pool, instances, terminateByPool=False):

        instance_ids_by_region = self._get_instance_ids_by_region(instances)
        ...
        for region in instance_ids_by_region:
            ...
            try:
                ...
                self.logger.exception("[Pool %d] terminate_pool_instances: boto failure: %s", pool.id, msg)
                return 1

    def start_instances(self, pool, config, prices, blacklisted_zones, amis, count=1):
        ...
        (region, zone, instance_type, rejected) = self._determine_best_price(config, prices, blacklisted_zones)
        ...
        try:
            ...
            if not userdata:
                self.logger.error("[Pool %d] Failed to compile userdata.", pool.id)
            ...
            try:
                ...
                self.logger.exception("[Pool %d] start_pool_instances: laniakea failure: %s", pool.id, msg)
                raise Exception({"type": "unclassified", "data": str(msg)})

            try:
                instances = {}
                self.logger.info("[Pool %d] Creating %dx %s instances... (%d cores total)", pool.id, count,
                ...
                for ec2_request in cluster.create_spot_requests(config.max_price * CORES_PER_INSTANCE[instance_type],
                    ...
                    instances[ec2_request]['pool'] = pool
                ...
                if "MaxSpotInstanceCountExceeded" in str(msg):
                    self.logger.warning("[Pool %d] start_pool_instances: Maximum instance count exceeded for region %s",
                                        pool.id, region)
                    raise Exception({"type": "max-spot-instance-count-exceeded", "data": "Auto-selected region \
                                     exceeded its maximum spot instance count."})
                elif "Service Unavailable" in str(msg):
                    self.logger.warning("[Pool %d] start_pool_instances: Temporary failure in region %s: %s",
                                        pool.id, region, msg)
                    raise Exception({"type": "temporary-failure", "data": "Temporary failure occurred: %s" % str(msg)})
                else:
                    self.logger.exception("[Pool %d] start_pool_instances: boto failure: %s", pool.id, msg)
                    raise Exception({"type": "unclassified", "data": "Unclassified error occurred: %s" % str(msg)})
        except Exception as msg:
            self.logger.exception("[Pool %d] start_pool_instances: unhandled failure: %s", pool.id, msg)
            raise Exception({"type": "unclassified", "data": "Unclassified error occurred: %s" % str(msg)})

    def check_instances_requests(self, region, pool, instances, tags):
        successful_requests = {}
        failed_requests = {}
        ...
        for req_id, result in zip(instances, results):
            if isinstance(result, boto.ec2.instance.Instance):
                self.logger.info("[Pool %d] spot request fulfilled %s -> %s", pool.id, req_id, result.id)
            ...
            # request object is returned in case request is closed/cancelled/failed
                ...
                if result.state in {"cancelled", "closed"}:
                    ...
                    self.logger.info("[Pool %d] spot request %s is %s", pool.id, req_id, result.state)
                    ...
                    self.logger.error("[Pool %d] %s", pool.id, msg)
                    failed_requests[req_id] = {}
                    failed_requests[req_id]['action'] = 'disable_pool'
                    failed_requests[req_id]['pool'] = pool.id
                    break
            elif result is None:
                self.logger.info("[Pool %d] spot request %s is still open", pool.pk, req_id)
            else:
                self.logger.warning("[Pool %d] spot request %s returned %s", pool.pk, req_id, type(result).__name__)
    ...
    def _get_instance_ids_by_region(self, instances):
            ...
            for region in prices[instance_type]:
                ...
                for zone in prices[instance_type][region]:
                    if (zone, instance_type) in blacklisted_zones:
                        ...
                        continue
                    out_prices = [price / CORES_PER_INSTANCE[instance_type] for price in
                                  prices[instance_type][region][zone]]
